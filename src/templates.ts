...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Profile-based "find bills for me" panel */
  form.find { display: flex; flex-wrap: wrap; gap: .5rem; margin: 0 0 1.25rem;
    padding: .85rem .9rem; border: 1px solid #8886; border-radius: 10px; background: #8881; }
  form.find label { flex-basis: 100%; font-size: .82rem; color: #888; margin: 0 0 .1rem; }
  form.find input[type=text] { flex: 1 1 260px; padding: .5rem .7rem; border: 1px solid #ccc;
    border-radius: 6px; font-size: .95rem; background: transparent; color: inherit; }
  form.find button { padding: .5rem 1.1rem; border: 1px solid #3b82f6; background: #3b82f6;
    color: white; border-radius: 6px; cursor: pointer; font-size: .95rem; font-weight: 600; }
  form.find button:hover { background: #2563eb; border-color: #2563eb; }
  .find-result { margin: 0 0 1.25rem; padding: .7rem .9rem;
    border-left: 3px solid #3b82f6; background: #3b82f61a;
    border-radius: 0 6px 6px 0; font-size: .88rem; }
  .find-result .matched-label { color: #888; margin-right: .4rem; }
  .find-result .matched-chips { display: inline-flex; flex-wrap: wrap; gap: .25rem;
    vertical-align: middle; }
  .find-result .matched-chips .tag-chip {
    background: #3b82f6; color: white; border-color: #3b82f6; font-weight: 600;
  }
  .find-result .clear-link { margin-left: .6rem; color: #888; text-decoration: underline; }
  .find-error { color: #c22d2d; margin: 0 0 1rem; font-size: .88rem; }
...
export interface RenderIndexOptions {
    ...
    total: number;
    /**
     * When set, we're rendering /find results: the profile is prefilled in
     * the find input, the matched tags are shown as a banner above the
     * cards, and the regular sort/search/tag-filter controls are hidden
     * (relevance already determined the result set).
     */
    findResult?: {
        profile: string;
        matchedTags: string[];
...
export function renderIndex(opts: RenderIndexOptions): string {
    const { bills, sortBy, order, q, tag, allTags, total, findResult } = opts;
    const isFind = Boolean(findResult);

    let emptyMsg: string;
    if (isFind) {
        emptyMsg = findResult?.error
            ? "Couldn't run the semantic match — see the error above. You can still browse all bills via the link."
            : "No bills matched your profile. Try different wording, or browse all bills.";
    } else if (tag) {
        emptyMsg = `Try clearing the "${esc(tag)}" tag filter.`;
    } else {
        emptyMsg = "The weekly pipeline will populate this page, or an admin can trigger <code>/admin/ingest</code>.";
    }
    const cards = bills.length
        ? bills.map(renderBillCard).join("\n")
        : `<div class="empty">${emptyMsg}${isFind ? ` <br><a href="/">Browse all bills →</a>` : ""}</div>`;

    const body = `
<h1>Congress One</h1>
<p class="tagline">Plain-English &amp; 简体中文 summaries of recent U.S. Congress bills. Cached, non-partisan, one screen each.</p>

${renderFindForm(findResult?.profile ?? "")}
${isFind ? renderFindResult(findResult!) : renderBrowseControls(sortBy, order, q, tag, allTags)}

<p class="meta">${bills.length} shown${isFind ? "" : ` · ${total} total cached${tag ? ` · filtered by tag <strong>${esc(tag)}</strong>` : ""}`}</p>
...
/**
 * Profile-based "find bills you might want to look at" form. Always visible
 * on both `/` and `/find` so users can discover the feature and refine a
 * prior query without navigating back.
 */
function renderFindForm(profile: string): string {
    return `<form class="find" method="get" action="/find">
  <label for="profile-input">Find bills for your profile</label>
  <input id="profile-input" type="text" name="profile"
         placeholder="e.g. Retired veteran in Texas, worried about healthcare costs and small business regulation"
         value="${esc(profile)}"
...
/**
 * Banner shown on the /find results page: lists the tags Gemini matched
 * from the user's profile, and provides a link to clear the search.
 */
function renderFindResult(fr: {
    profile: string;
    matchedTags: string[];
    error: string | null;
}): string {
    if (fr.error) {
        return `<p class="find-error">Couldn't run semantic search: ${esc(fr.error)}</p>`;
    }
    if (fr.matchedTags.length === 0) {
        return `<div class="find-result">
  <span class="matched-label">No tags in the current cache matched your profile.</span>
  <a class="clear-link" href="/">clear search</a>
</div>`;
    }
    const chips = fr.matchedTags
        .map(
            (t) =>
        ...
        .join("");
    return `<div class="find-result">
  <span class="matched-label">Matched topics:</span>
  <span class="matched-chips">${chips}</span>
  <a class="clear-link" href="/">clear search</a>
</div>`;
}

/**
 * The classic browse controls (sort / order / substring search) + the tag
 * filter bar. Hidden on /find results because relevance determines the set.
 */
